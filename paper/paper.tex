...
We only applied stacking on task 2 as task 1 is too compute-intensive.
...
\textbf{Fig. 1} shows the AUC perfomance of Local 5-folds Cross Validation and 30\% of testing data using same parameters and seed.
 There is a strong correlation in AUC between Cross Validation and 30\% of testing data.
 The AUC improved once stacking is applied. We included more randomized stacking features until there is no improvement in AUC.
...
We applied grid search on a subset of parameters. In general, $max\_depth$, $min\_child\_weight$, $gamma$, $lambda$ and $alpha$ are tuned for model complexity,
 ...
 For the parameter combinations with top 5\% ranking, we re-trained the model using a lower $eta$ and selected the one with best Cross Validation.
