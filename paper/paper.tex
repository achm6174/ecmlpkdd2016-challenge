...
We only applied stacking on task 2.
We tried two stacking method seperately
\begin{enumerate}
 \item 2-folds stacking split by two different time frame

 \item 10-folds stacking after merging two time frame
\end{enumerate}

Since stacking index must be fixed in order to avoid leakage, we can only choose either 2-folds or 10-folds.
Although 10 folds is a common choice as it provides minmial information loss, we finally choose two folds for two reasons:
\begin{enumerate}
 \item The training data is a combination data in two periods. It provides out-of-period stacking feature split by time, which is useful to avoid overfitting.

 \item Both stacking provides improvement on local CV, but 10 folds show discrepancy betweem CV and 30\% of testing data.
\end{enumerate}

In practice, a diversified combination of stacking features from different algorithms (KNN, RandomForest, SVM, NN, etc.)
will outperform stacking using only one algorithm.
Since we only used XGboost here, we tried to compensate this by building several stacking features using randomized parameters to achieve diversification.


\textbf{Fig. 1} shows the AUC perfomance of Local 5-folds Cross Validation result and 30\% of testing data using same parameters and seed.
 Although there's a difference in AUC between local CV and 30\% of testing data,
 there is a strong correlation on AUC between CV and testing data. The AUC improved when stacking is applied and result keep improving
 when more randomized stacking features is used.

\begin{figure}[ht!]
\centering
\includegraphics[width=125mm]{Rplot.png}
...
We applied grid search on a subset of parameters. In general, $max\_depth$, $min\_child\_weight$, $gamma$, $lambda$ and $alpha$ are tuned for model complexity,
 ...
 In order to speed things up we used a high $eta$ (shrinkage) in the beginning and ranked parameter combinations based on cross validation score.
...
\begin{table}[h!]
  ...
  \begin{tabular}{p{3cm}p{3cm}p{4cm}p{2cm}}
    ...
    \midrule
    $eta$ & Shrinkage in update & [0.005, 0.01, 0.05, 0.1] & 0.005 \\
    $max\_depth$ & Maximum depth of tree & [3, 4, 5, 6, 7, 8] & 3 \\
    ...
    $subsample$ & Subsample of data & [0.7, 0.8, 0.9, 1] & 1 \\
