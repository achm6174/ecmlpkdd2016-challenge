...
will usually outperform stacking using only one algorithm.
Since we only used xgboost here, we tried to compensate this by building several stacking features using randomized parameters.


Fig. 1 shows the AUC perfomance of CV result and 30\% of testing data. Although there's a difference of AUC between local CV and 30\% of testing data,
there is a strong correlation between CV and testing data. The AUC is higher when stacking is applied and result keep improving when we used more stacking features.

\begin{figure}[ht!]
\centering
\includegraphics[width=110mm]{Rplot.png}
\caption{AUC of 5folds CV result and 30\% testing data using different number of stacking features \label{overflow}}
\end{figure}
